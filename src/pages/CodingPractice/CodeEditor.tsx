...
import {
  ...
  Play,
  XCircle,
  Settings // Used for the dropdown icon
} from 'lucide-react'
...
const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  // 2. State for Selected Language (Defaults to prop or 'python')
  const [selectedLanguage, setSelectedLanguage] = useState(
    externalLanguage || question?.programming_language || 'python'
  )
  ...
  // 3. Helper to get Starter Code for each language
...
int main() {
  ...
  // 4. Effect: Update code when Language Changes
  useEffect(() => {
    // Only reset code if the language actually changed to avoid overwriting user progress on initial load
    // For simplicity, we set it to starter code whenever the language selector changes.
  ...
  const runCode = async () => {
    ...
    try {
      // 5. Execute using selectedLanguage
      const executionResult = await executeCodeWithHiddenTests(
        question?.id || '',
        code,
        selectedLanguage // <--- Uses the dropdown value
      )
    ...
    <div className="space-y-6 relative">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold">Write Code</h3>
        ...
        <Editor
          height="540px"
          language={selectedLanguage} // <--- Updates editor syntax highlighting
          theme="vs-dark"
          value={code}
                    ...
                    return (
                      ...
                      >
                        {test.passed ? (
                          ...
                          <XCircle className={`w-4 h-4 ${isActive ? 'text-white' : 'text-red-600'}`} />
                        )}
                        Case {test.index}
                      </button>
                    );
